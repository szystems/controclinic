<div class="py-6">
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="flex items-center gap-3 mb-6">
            ...
            <div>
                <h1 class="text-2xl font-bold text-gray-900 dark:text-white">{{ __('invoices.edit_invoice') }}</h1>
                <p class="text-xs text-gray-500 dark:text-gray-400">
                    {{ $invoice->invoice_number }} · {{ __('invoices.currency') }}: <span class="font-mono font-semibold">{{ $invoice->currency }}</span>
                </p>
            </div>
        </div>

        @if(session('success'))
            <div class="mb-4 px-4 py-3 bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-200 dark:border-emerald-800 rounded-lg text-sm text-emerald-700 dark:text-emerald-300">
                {{ session('success') }}
            </div>
        @endif
        ...
        <form wire:submit="save" class="space-y-6">
            ...
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-6 space-y-5">
                ...
                {{-- Paciente (solo lectura en edición) --}}
                <div>
                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">{{ __('invoices.patient') }}</label>
                    <div class="flex items-center gap-3 px-4 py-3 bg-gray-50 dark:bg-gray-700/50 border border-gray-200 dark:border-gray-600 rounded-lg">
                        ...
                        <div>
                            ...
                            @if($invoice->patient?->email || $invoice->patient?->phone)
                                <p class="text-xs text-gray-500 dark:text-gray-400">
                                    {{ implode(' · ', array_filter([$invoice->patient->email ?? '', $invoice->patient->phone ?? ''])) }}
                                </p>
                            @endif
                        </div>
                        <span class="ml-auto text-xs text-gray-400 dark:text-gray-500 italic">{{ __('invoices.patient_locked') }}</span>
                    </div>
                </div>
                        ...
                        <div class="px-6 py-5 space-y-4" wire:key="item-{{ $i }}">
                            ...
                            <div>
                                ...
                                <div class="flex rounded-lg border border-gray-300 dark:border-gray-600 overflow-hidden focus-within:ring-2 focus-within:ring-indigo-500 focus-within:border-indigo-500">
                                    <span class="flex items-center px-3 bg-gray-50 dark:bg-gray-700 border-r border-gray-300 dark:border-gray-600 text-xs font-mono text-gray-500 dark:text-gray-400 select-none">{{ $invoice->currency }}</span>
                            ...
                            <div class="flex items-center justify-between pt-3 border-t border-gray-100 dark:border-gray-700">
                                ...
                                <span class="text-sm font-bold text-gray-900 dark:text-white tabular-nums">{{ $invoice->currency }} {{ number_format($lineTotal, 2) }}</span>
                            </div>
                        </div>
                    @endforeach
                </div>
                ...
                <div class="px-6 py-4 bg-gray-50 dark:bg-gray-900/30 border-t border-gray-100 dark:border-gray-700">
                    <dl class="ml-auto w-full sm:w-72 space-y-1.5 text-sm">
                        <div class="flex justify-between">
                            ...
                            <dd class="tabular-nums text-gray-900 dark:text-white">{{ $invoice->currency }} {{ number_format($breakdown['subtotal'], 2) }}</dd>
                        </div>
                        @if($breakdown['discount'] > 0)
                            <div class="flex justify-between">
                                <dt class="text-gray-500 dark:text-gray-400">{{ __('invoices.discount') }}</dt>
                                <dd class="tabular-nums text-rose-600 dark:text-rose-400">− {{ $invoice->currency }} {{ number_format($breakdown['discount'], 2) }}</dd>
                            </div>
                        @endif
                        @if($breakdown['tax'] > 0)
                            <div class="flex justify-between">
                                <dt class="text-gray-500 dark:text-gray-400">{{ __('invoices.tax') }}</dt>
                                <dd class="tabular-nums text-gray-900 dark:text-white">{{ $invoice->currency }} {{ number_format($breakdown['tax'], 2) }}</dd>
                            </div>
                        @endif
                        <div class="flex justify-between pt-2 border-t border-gray-200 dark:border-gray-700">
                            <dt class="font-semibold text-gray-900 dark:text-white">{{ __('invoices.total') }}</dt>
                            <dd class="tabular-nums font-bold text-base text-gray-900 dark:text-white">{{ $invoice->currency }} {{ number_format($breakdown['total'], 2) }}</dd>
                        </div>
                    </dl>
                </div>
            </div>
